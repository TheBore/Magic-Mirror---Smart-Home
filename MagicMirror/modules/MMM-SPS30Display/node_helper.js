...
module.exports = NodeHelper.create({
  ...
  socketNotificationReceived(notification, payload) {
    if (notification === "INIT_WEBSOCKET_SPS30") {
      const wsUrl = payload;
      ...
      this.ws.on("message", (message) => {
        ...
        try {
          ...
          this.sendSocketNotification("SPS30_DATA", data);
        } catch (e) {
      ...
      this.ws.on("close", () => {
        ...
        this.reconnectTimer = setTimeout(() => {
          this.reconnectTimer = null;
          if (this.wsUrl) {
            this.socketNotificationReceived("INIT_WEBSOCKET_SPS30", this.wsUrl);
          }
        }, 5000);
      });
    }
  ...
  stop() {
    this.log("Stopping MMM-SPS30Display node helper...");
    this.cleanupWebSocket();
    this.wsUrl = null;
  },

  log(...args) {
    console.log("[MMM-SPS30Display]", ...args);
  }
});
